...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
  ...
  <DocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
    <Author>Salted Margin Command Center</Author>
    <Company>Salted prototype</Company>
...
function worksheetXml({ title, subtitle, meta, table, summaryCards, sections, includeIntro, sheetName }) {
  ...
  const body = [];
  body.push(rowXml([cellXml("salted", "sTitle", "String", colCount - 1)], 30));
  body.push(rowXml([cellXml(`Margin Command Center - ${title}`, "sSubtitle", "String", colCount - 1)], 22));
  body.push(rowXml([]));
  body.push(rowXml([
...
function inferType(key, value = "") {
  if (key.includes("margin_pct") || key.includes("refund_rate") || key.includes("mix")) return "percent";
...
function toneFor(type, value) {
  ...
  if (["medium", "in progress", "queued"].includes(text) || text.includes("reprice") || text.includes("reduce promo")) return "medium";
